// =============================================================================
//  scripts/aggregate-clicks.js  ―  クリックログ集計バッチ
// =============================================================================
//
...
//             │
//             ▼
//     ③ 辞書再構築（rebuild-trie-data.js）  元辞書に集計結果を反映する
...
async function readJson(fileName, fallback) {
  try {
    ...
    console.warn(`[aggregate] ${fileName} を読めませんでした（${err.message}）。既定値を使います。`);
    return fallback;
  }
}
...
async function main() {
  // ① 行動ログ と 元辞書 を読む
  const logs = await readJson('click-logs.json', []);
  const words = await readJson('words.json', []);

  // term -> baseScore（元のスコア）の対応表を作る
  const baseScoreByTerm = new Map();
  ...
  for (const [term, clicks] of clicksByTerm) {
    const baseScore = baseScoreByTerm.has(term) ? baseScoreByTerm.get(term) : 0;
